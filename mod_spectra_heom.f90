...
real*8 tolerance
real*8,allocatable:: sum_c_over_nu(:)
...
integer nquant
real*8 temperature
real*8,allocatable:: gamma(:),eta(:),lambda(:)
...
subroutine setup
  ...
  read(10,*)
  allocate(gamma(nquant))
  do i=1,nquant
    read(10,*) gamma(i)
  enddo
  read(10,*)
  read(10,*)
  allocate(lambda(nquant),eta(nquant))
  do i=1,nquant
    read(10,*) lambda(i)
  enddo
  read(10,*)
...
!  eta=2*lambda/hbar
  ...
  allocate(c_matsubara(nquant,0:KK),omg_matsubara(nquant,0:KK))
  allocate(sum_c_over_nu(nquant))
...
subroutine compute_deriv_n(n,rho,drho_dt)
  ...
  integer nvec(nquant,0:KK)!,nvec_plus(nquant,KK),nvec_neg(nquant,KK)
    ...
    if(zero(n)==0) then !! matrix at n is not filtered out
      ...
      do m=1,nquant
        do k=0,KK
          tmp=tmp-nvec(m,k)*omg_matsubara(m,k)*rho(:,:,n)
        enddo
      enddo
      ...
      if(flag_truncate==1) then
        do m=1,nquant
          ...
          tmp=tmp-sum_c_over_nu(m)*commute(mat_tmp,commute(mat_tmp,rho(:,:,n)))
        enddo
      endif
    endif
    ...
    do m=1,nquant
      ...
      do k=0,KK
        ...
        if(nplus>0.and.nplus<=nn_tot) then
          ...
          sqrt((nvec(m,k)+1.d0)*abs(c_matsubara(m,k)))
      ...
      do k=0,KK
        ...
        if(nminus>0.and.nminus<=nn_tot) then
          if(zero(nminus)==0)then
            !! Scaled HEOM approach from JCP 130, 084105 (2009)
            if(flag_scale==1)tmp=tmp-iota*(c_matsubara(m,k)*matmul(mat_tmp,rho(:,:,nminus)) &
- dconjg(c_matsubara(m,k))*matmul(rho(:,:,nminus),mat_tmp) )*sqrt(nvec(m,k)/abs(c_matsubara(m,k)))
            if(flag_scale==0)tmp=tmp-iota*nvec(m,k)*(c_matsubara(m,k)*matmul(mat_tmp,rho(:,:,nminus)) &
- dconjg(c_matsubara(m,k))*matmul(rho(:,:,nminus),mat_tmp) )
